...
window.addEventListener('swal', function (event) {
  sweetalert2__WEBPACK_IMPORTED_MODULE_1___default().fire(event.detail);
}); // Show and Hide Edit Modal Task

var editModal = document.getElementById('editForm');

if (editModal) {
  var editModalBootstrap = new (bootstrap_dist_js_bootstrap_bundle__WEBPACK_IMPORTED_MODULE_0___default().Modal)(editModal);
}

Livewire.on('showEditModal', function () {
  editModalBootstrap.show();
});
Livewire.on('hideEditModal', function () {
